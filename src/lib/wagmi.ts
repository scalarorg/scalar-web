...
export const isSupportedChain = (chainId: number): chainId is (typeof chains)[number]['id'] => {
  return chains.map((chain) => chain.id).includes(chainId as (typeof chains)[number]['id']);
};
